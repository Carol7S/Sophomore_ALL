...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam){
    ...
    switch(message){
        ...
        case WM_PAINT:
            ...
            for(n=0; n<51; n++, i+=3, j+=5) {//255 255 0
                hP = (HPEN) CreatePen(PS_SOLID, 3, RGB(j, 255, 0));
                hPenOld = (HPEN) SelectObject(hdc, hP);
                MoveToEx(hdc, (rect.left + rect.right) / 2 - 50, rect.top+i, lpPoint);
                LineTo(hdc, (rect.left + rect.right) / 2 + 50, rect.top+i);
            }
            for(n=0; n<51; n++, i+=3, k-=5){//255 0 0
                hP = (HPEN) CreatePen(PS_SOLID, 3, RGB(255, k, 0));
                hPenOld = (HPEN) SelectObject(hdc, hP);
                MoveToEx(hdc, (rect.left + rect.right) / 2 - 50, rect.top+i, lpPoint);
                LineTo(hdc, (rect.left + rect.right) / 2 + 50, rect.top+i);
            }

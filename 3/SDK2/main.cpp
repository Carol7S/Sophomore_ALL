					...
					LONG lParam)			//消息处理函数.
{
	...
	case WM_PAINT:					//处理绘图消息.
		...
		for(i=0;i<nChar;i+=2)
		{
            ...
            TextOut(hDC,X,Y,&lpsz_1[i+1],1);	//输出从第1个到第nChar个字符.
		}
		...
		X=X+tm.tmAveCharWidth*2;
		TextOut(hDC,X,Y,&lpsz_1[nChar+1],1);	//输出后面的字符.

		Y=tm.tmExternalLeading+10;
		for(i=nChar+2;i<nCharlen;i+=2)
		{
			SetTextColor(hDC,RGB(0,0,0));	//设置字体的颜色为黑色.
			X=X+tm.tmAveCharWidth*2;		//设置输出字符的X坐标.
			TextOut(hDC,X,Y,&lpsz_1[i],1);	//输出后面的字符.
            X=X+tm.tmAveCharWidth*2;
            TextOut(hDC,X,Y,&lpsz_1[i+1],1);	//输出后面的字符.
		}

		if(nChar == nCharlen-2)				//当输出到最后的一个字符时,
		{
...
BOOL InitWindowsClass(HINSTANCE hInstance)
{
	...
	WndClass.lpfnWndProc= reinterpret_cast<WNDPROC>(WndProc);			//消息处理函数.
